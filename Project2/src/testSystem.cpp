...
class SysTest : public ::testing::Test {
    protected:
        ...
        vector<string> fileChoice;
        vector<string> multFiles;
    ...
    void SetUp() override {
        ...
        multFiles = {"../testing/STV6.csv\n", "../testing/Plurality6.csv\n", "../testing/MV6.csv\n",
                     "../testing/STV7.csv\n", "../testing/Plurality7.csv\n", "../testing/MV7.csv\n",
                     "../testing/STV8.csv\n", "../testing/Plurality8.csv\n", "../testing/MV8.csv\n",
                     "../testing/STV9.csv\n", "../testing/Plurality9.csv\n", "../testing/MV9.csv\n",
                     "../testing/STV10.csv\n", "../testing/Plurality10.csv\n", "../testing/MV10.csv\n"};
        fileChoice = {"1\n", "2\n"};
    }
...
TEST_F(SysTest, fairElectionValidityPlurality) { // Test Case ID#: 31
    ...
    resultsS.erase(resultsS.begin(), resultsS.begin() + 6);
...
TEST_F(SysTest, seatValidityDefefictMV) { // Test Case ID#: 48 
    ...
    vector<string> subWinners(results.begin() + 5, results.begin() + 25);
    vector<string> subLosers(results.begin() + 26, results.begin() + 26);
    vector<string> subVoteCount(results.begin() + 27, results.end());

    sort(subWinners.begin(), subWinners.end());
    sort(subVoteCount.begin(), subVoteCount.end());

    EXPECT_EQ(results.at(0), "Election type: Municipal");
    EXPECT_EQ(results.at(1), "Number of seats: 21");
    EXPECT_EQ(results.at(2), "Number of ballots: 1");
    EXPECT_EQ(results.at(3), "Number of candidates: 20");
    EXPECT_EQ(results.at(4), "Winners:");
    EXPECT_EQ(results.at(25), "Losers:");
    EXPECT_EQ(results.at(26), "Number of votes:");
    for (int i = 0; i < 20; i++) {
        if (subWinners.at(i) == "Leon Kennedy") {
            EXPECT_EQ(subWinners.at(i) + ": 1", subVoteCount.at(i));
        } else {
            EXPECT_EQ(subWinners.at(i) + ": 0", subVoteCount.at(i));
        }
    }
    delete election;
    delete ballots;
...
TEST_F(SysTest, seatValiditySurplusMV) { // Test Case ID#: 49
    ...
    Municipal* MVelection = static_cast<Municipal*>(election);

    int winners_size = MVelection->getWinners().size();
    int losers_size = MVelection->getLosers().size();
    int candidates_size = MVelection->getCandidates().size();
...
TEST_F(SysTest, fairElectionValidityMV) { // Test Case ID#: 50
    ...
    vector<string> subWinners(results.begin() + 5, results.begin() + 10);
    vector<string> subLosers(results.begin() + 11, results.begin() + 16);
    vector<string> subVoteCount(results.begin() + 17, results.end());
    ...
    EXPECT_EQ(results.at(4), "Winners:");
    EXPECT_EQ(results.at(10), "Losers:");
    EXPECT_EQ(results.at(16), "Number of votes:");
    EXPECT_EQ(subWinners.size(), 5);
    EXPECT_EQ(subLosers.size(), 5);
    EXPECT_EQ(subVoteCount.size(), 10);
    ...
    vector<string> subWinnersS(resultsS.begin() + 5, resultsS.begin() + 10);
    vector<string> subLosersS(resultsS.begin() + 11, resultsS.begin() + 16);
    vector<string> subVoteCountS(resultsS.begin() + 17, resultsS.end());

    sort(subWinnersS.begin(), subWinnersS.end());
    sort(subLosersS.begin(), subLosersS.end());
    sort(subVoteCountS.begin(), subVoteCountS.end());

    EXPECT_EQ(resultsS.at(0), "Election type: Municipal");
    EXPECT_EQ(resultsS.at(1), "Number of seats: 5");
    EXPECT_EQ(resultsS.at(2), "Number of ballots: 1000");
    EXPECT_EQ(resultsS.at(3), "Number of candidates: 10");
    EXPECT_EQ(resultsS.at(4), "Winners:");
    EXPECT_EQ(resultsS.at(10), "Losers:");
    EXPECT_EQ(resultsS.at(16), "Number of votes:");
    EXPECT_EQ(subWinnersS.size(), 5);
    EXPECT_EQ(subLosersS.size(), 5);
    EXPECT_EQ(subVoteCountS.size(), 10);
    for (int i = 0; i < static_cast<int>(subWinners.size()); i++) {
        EXPECT_EQ(subWinners.at(i), subWinnersS.at(i));
    }
    for (int i = 0; i < static_cast<int>(subLosers.size()); i++) {
    ...
    for (int i = 0; i < static_cast<int>(subVoteCountS.size()); i++) {
        EXPECT_EQ(subVoteCountS.at(i), subVoteCountS.at(i));
    }
    delete election;
    delete ballots;
    delete electionShuffle;
...
TEST_F(SysTest, regularMV) { // Test Case ID#: 52
    ...
    vector<Candidate> winners = MVelection->getWinners();
    vector<Candidate> losers = MVelection->getLosers();
    int seatNum = MVelection->getSeats();
    int loser_size = MVelection->getCandidates().size() - seatNum;
    EXPECT_EQ(winners.size(), seatNum);
    EXPECT_EQ(losers.size(), loser_size);
    for (long unsigned int i = 0; i < winners.size(); i++) {
        for (long unsigned int j = 0; j < losers.size(); j++) {
...
TEST_F(SysTest, tieMV) { // Test Case ID#: 53
    ...
    vector<string> subWinners(results.begin() + 5, results.begin() + 8);
    vector<string> subLosers(results.begin() + 9, results.begin() + 11);
    vector<string> subVoteCount(results.begin() + 12, results.begin() + 17);
    ...
    EXPECT_EQ(results.at(4), "Winners:");
    EXPECT_EQ(results.at(8), "Losers:");
    EXPECT_EQ(results.at(11), "Number of votes:");
    EXPECT_EQ(subWinners.size(), 3);
    EXPECT_EQ(subLosers.size(), 2);
    EXPECT_EQ(subVoteCount.size(), 5);
    for (int i = 0; i < static_cast<int>(subVoteCount.size()); i++) {
        string percent = subVoteCount.at(i).substr(subVoteCount.at(i).length() - 3, 3);
...
TEST_F(SysTest, timeMV) { // Test Case ID#: 54
    ...
    vector<string> user_input;
    user_input.push_back(file_names.at(20));
    user_input.push_back(fileChoice.at(1));
...
TEST_F(SysTest, multFilePV) { // Test Case ID#: 55
    ...
    user_input.push_back(multFiles.at(4));
    user_input.push_back(fileChoice.at(0));
    user_input.push_back(multFiles.at(7));
    user_input.push_back(fileChoice.at(0));
    user_input.push_back(multFiles.at(10));
    user_input.push_back(fileChoice.at(0));
    user_input.push_back(multFiles.at(13));
    user_input.push_back(fileChoice.at(1));
...
TEST_F(SysTest, multFileSTV) { // Test Case ID#: 56
    ...
    user_input.push_back(fileChoice.at(0));
    user_input.push_back(multFiles.at(3));
    user_input.push_back(fileChoice.at(0));
    user_input.push_back(multFiles.at(6));
    user_input.push_back(fileChoice.at(0));
    user_input.push_back(multFiles.at(9));
    user_input.push_back(fileChoice.at(0));
    user_input.push_back(multFiles.at(12));
    user_input.push_back(fileChoice.at(1));

    user_input.push_back(auditFile.at(8));
    ...
    bool access(auditFile.at(8).c_str());
    EXPECT_TRUE(access);
    delete election;
    delete ballots;
};
...
TEST_F(SysTest, multFileMV) { // Test Case ID#: 57
    ...
    user_input.push_back(fileChoice.at(0));
    user_input.push_back(multFiles.at(5));
    user_input.push_back(fileChoice.at(0));
    user_input.push_back(multFiles.at(8));
    user_input.push_back(fileChoice.at(0));
    user_input.push_back(multFiles.at(11));
    user_input.push_back(fileChoice.at(0));
    user_input.push_back(multFiles.at(14));
    user_input.push_back(fileChoice.at(1));
