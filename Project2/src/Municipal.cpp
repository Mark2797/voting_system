...
static void countVotes(Ballots*& ballots, std::vector<Candidate>& candidates) {
    for (int i = 0; i < ballots->getBallotCount(); i++) {
        ...
        for (int j = 0; j < static_cast<int>(currentBallot.size()); j++) {
            if (currentBallot.at(j) == 1) {
                candidates.at(j).assignBallot(i);
            }
        }
    }
}

// generates a random number between 0 and the argument, exclusive
static int generateWinnerIndex(int end) {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_int_distribution<> distrib(0, end - 1);
    return distrib(gen);
}

// finds the winners of this municipal election and populates this->winners
static void determineWinners(int& winnerIndex, int& mostVotes, std::vector<Candidate>& losers, std::vector<Candidate>& potentialWinners, std::vector<Candidate>& winners) {
    ...
    if (potentialWinners.size() > 1) {
        int chosenIndex = generateWinnerIndex(potentialWinners.size());
...
Municipal::Municipal(Ballots* ballots, int seats) : Election(ballots, seats) {
    this->candidates = {};
...
void Municipal::displayElectionDetails() {
    std::cout << "Election type: Municipal" << std::endl;
    ...
    std::cout << "Number of votes:" << std::endl;
    for (long unsigned int i = 0; i < this->candidates.size(); i++) {
        std::cout << this->candidates.at(i).getName() << ": " << this->candidates.at(i).getBallotNum() << std::endl;
    }
}
